...
// arr.indexOf(item, from)는 인텍스 from부터 시작해 item(요소)를 찾는다.
...
// 위 메서드들은 요소를 찾을때 완전 항동 연산자 === 을 사용한다는 점에 유의해야 한다.
...
const arr2 = [NaN];
console.log( arr2.indexOf(arr2) );  // -1   (완전 항동 비교 === 는 NaN엔 동작하지 않으므로 0이 출력되지 않는다.)
console.log( arr2.includes(arr2) ); // true (NaN의 여부를 확인한다.)
...
console.log(results);   // [ 12, 8, 44 ]


/** 배열 정렬 */
const arr5 = [2, 1, 15];
// 큌정렬 알고리즘을 사용하여 배열 자체를 정렬한다.
// --> 배열의 모든 원소를 문자열로 취급하기 때문에 글자 정렬기준이 적용된다.
// arr5.sort();
// console.log(arr5);  // [ 1, 15, 2 ]

// sort 함수도 정렬조건을 콜백함수로 처리한다.
