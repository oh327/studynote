/**
    ...
    2. Rumtime Error : 프로그램 작성 과정에서 논리상의 오류로 미처 대응하지 못한 상황이 발생하는 경우.
...
// 에러가 있다면, try 안의 코드의 실행이 중단되고, catch(err) 블록으로 제어 흐름이 넘어간다.
// 변수 err(아무 이름이나 사용 가능)는 무슨일이 일어났는지에 대한 설명이 담긴 에러 객체를 포함한다.
try {
    for (let i=0; i<10; i++) {
